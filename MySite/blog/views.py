...
def post_list(request, tag_slug=None):
    ...
    ### постраничная разбивка с 3 постами на страницу (Paginator)
...
def post_detail(request, year, month, day, post):
    ...
    if request.method == "POST":
        ...
        comment_form = CommentForm()                    ###  Форма для комментариев пользователей
...
def post_share(request, post_id):
    post = get_object_or_404(Post, id=post_id, status=Post.STATUS_CHOICES)
    if request.method == "POST":
